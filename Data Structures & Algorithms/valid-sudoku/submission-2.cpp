...
bool check_row(vector<vector<char>>& board){
    ...
    for(int i=0;i<board.size();i++){
        ...
        for(int j=0;j<board[0].size();j++){
            if(board[i][j]=='.')
                continue;
            else if(my_row.find(board[i][j])!=my_row.end())
                return false;
            else
...
bool check_column(vector<vector<char>>& board){
    ...
    for(int j=0;j<board[0].size();j++){
        ...
        for(int i=0;i<board.size();i++){
            if(board[i][j]=='.')
                continue;
            else if(my_column.find(board[i][j])!=my_column.end())
                return false;
            else
...
bool check_box(vector<vector<char>>& board){ 
    ...
    for(int start_row=0;start_row<9;start_row+=3){
        for(int start_column=0;start_column<9;start_column+=3){
            ...
            for(int i=0;i<3;i++){
                for(int j=0;j<3;j++){
                    ...
                    char current = board[start_row+i][start_column+j];

                    if(current=='.')
                        continue;

                    else if(my_box.find(current)!=my_box.end())
                        return false;
